...
const Education = () => {
  ...
  if (localStorage.getItem("isChanged") == "true") {
    if (
      ...
      JSON.parse(localStorage.getItem("userData"))["education"] != null
    )
      education = JSON.parse(localStorage.getItem("userData"))["education"];
    else education = [];
  } else {
    ({ education } = userData);
  }
                ...
                <Card className={styles.eduCardMainBack}>
                  ...
                  <Card.Body className={styles.eduCardMain}>
                    <Card.Text>
                      {item[7].map((course) => {
                        return (
                          <Badge className={styles.courseBadge}>{course}</Badge>
                        );
                      })}
                    </Card.Text>
                    <Card.Text>
